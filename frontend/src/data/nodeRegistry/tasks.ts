...
export const nodes: Record<string, NodeMeta> = {
  "task.define": {
    ...
    autoExpand: true,
    ports: [
      { id: "slot:body", side: "output", label: "Body", icon: "task", type: "flow" }
    ],
    docs: {
      summary: "Create a named callable task with its own entry branch and parameter schema.",
      usage:
  ...
  "task.call": {
    ...
    color: TASK_COLOR,
    ports: [
      { id: "input:args", side: "input", label: "Args", icon: "task", type: "Record<string, unknown>", preview: true }
    ],
    docs: {
      ...
      inputs: "- **Args:** Optional record of runtime arguments overriding defaults.",
      notes: "Compilation validates that the task name exists in the current file."
    },
    codegen: { emitter: "task" }
  }
};
