...
export const nodes: Record<string, NodeMeta> = {
  "structure.spawn": {
    ...
    color: STRUCT_COLOR,
    ports: [
      { id: "input:spawn", side: "input", label: "Spawn", icon: "structure", type: "StructureSpawn", required: false }
    ],
    docs: {
      summary: "Spawn a new creep using the configured body strategy and memory template.",
      usage:
      ...
      outputs: "- **Result:** Emits flow success or failure via the node status.",
      notes: "Respects energy cap and room control level when building the body."
    },
  ...
  "structure.tower": {
    ...
    color: STRUCT_COLOR,
    ports: [
      { id: "input:tower", side: "input", label: "Tower", icon: "tower", type: "StructureTower", required: false }
    ],
    docs: {
      summary: "Configure priorities for tower healing, attacking, and repairing.",
      usage:
  ...
  "structure.linkSend": {
    ...
    color: STRUCT_COLOR,
    ports: [
      { id: "input:from", side: "input", label: "From", icon: "link", type: "StructureLink", required: true },
      { id: "input:to", side: "input", label: "To", icon: "link", type: "StructureLink", required: true }
    ],
    docs: {
  ...
  "structure.terminalMarket": {
    ...
    color: STRUCT_COLOR,
    ports: [
      { id: "input:terminal", side: "input", label: "Terminal", icon: "terminal", type: "StructureTerminal", required: false }
    ],
    docs: {
      ...
      inputs: "- **Terminal:** Optional terminal structure executing the trade.",
      notes: "Automatically skips when orders cannot be fulfilled or cooldown prevents usage."
    },
    codegen: { emitter: "structure" }
  }
};
