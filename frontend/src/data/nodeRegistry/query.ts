...
export const nodes: Record<string, NodeMeta> = {
  "query.find": {
    ...
    docs: {
      ...
      inputs:
        "- **Room:** Optional room context when querying outside the current room.",
      outputs:
        "- **Result:** Array or single object depending on the mode configured in the node.",
  ...
  "query.sortBest": {
    ...
    docs: {
      ...
      notes:
        "When configured for 'all' mode the original list is sorted in-place.",
    },
    codegen: { emitter: "query" },
  },
};
