...
const compileTasks = (file: FileIR, ctx: EmitContext, warnings: WarningMessage[]): string[] => {
  if (!file.taskDefs) {
    return [];
  }
  ...
  return file.taskDefs.map((task) => {
    ...
    const paramsComment = task.params
      ?.map((param) => `${param.key}: ${param.type}${param.default !== undefined ? `=${JSON.stringify(param.default)}` : ''}`)
      .join(', ');
...
export const compileFile = (file: FileIR): CompilerResult => {
  ...
  const code = wrapRoleFile(ctx.getImportLines(), ctx.getStatements(), taskBlocks);
...
export const compileGraphToCode = (fileId: string, graph: Parameters<typeof buildFileIR>[1]): CompilerResult => {
