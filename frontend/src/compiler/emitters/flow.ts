...
export const emitIf = (
  ...
  const condExpr = ctx.resolvePort(
    node.id,
    node.inputs.cond,
    node.config.condition ?? "false",
  );
...
export const emitSwitch = (
  ...
  const keyExpr = ctx.resolvePort(
    node.id,
    node.inputs.key,
    node.config.key ?? "undefined",
  );
  const cases = Array.isArray(node.config.cases)
    ? (node.config.cases as Array<{ label?: string; matchValue: unknown }>)
    : [];
  ...
  cases.forEach((caseConfig, index) => {
    ...
    const label =
      caseConfig.matchValue !== undefined
        ? JSON.stringify(caseConfig.matchValue)
        : "undefined";
...
export const emitLoop = (
  ...
  const loopType =
    typeof node.config.loopType === "string" ? node.config.loopType : "while";
  ...
  if (loopType === "for") {
    const iterName =
      typeof node.config.iterVarName === "string" &&
      node.config.iterVarName.trim().length > 0
        ? node.config.iterVarName.trim()
        : ctx.newTemp("i");
    ctx.pushStatement(
      `for (let ${iterName} = 0; ${iterName} < ${maxIterations}; ${iterName} += 1) {`,
    );
    ctx.enterScope();
...
export const emitTry = (
  ...
  const errorVar =
    typeof node.config.errorVar === "string" &&
    node.config.errorVar.trim().length > 0
      ? node.config.errorVar
      : "err";
