...
export const emitSpawnCreep = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  const spawnExpr = ctx.resolvePort(
    node.id,
    node.inputs.spawnRef,
    'creep.room.find(FIND_MY_SPAWNS)[0]'
  );
  const namePrefix = typeof node.config.namePrefix === 'string' ? node.config.namePrefix : 'worker';
  const memoryTemplate =
    typeof node.config.memoryTemplate === 'object' && node.config.memoryTemplate !== null
      ? (node.config.memoryTemplate as Record<string, unknown>)
      : {};
  if (typeof node.config.roleFile === 'string' && node.config.roleFile.length > 0) {
    memoryTemplate.role = node.config.roleFile;
  }
  const strategy = typeof node.config.bodyStrategy === 'string' ? node.config.bodyStrategy : 'Fixed';
  const weights =
  ...
  ctx.pushStatement('{');
  ctx.enterScope();
  ctx.pushStatement(`const spawn = ${spawnExpr} as StructureSpawn | undefined;`);
  ctx.pushStatement('if (spawn) {');
  ctx.enterScope();
  ctx.pushStatement('const body = [] as BodyPartConstant[];');

  if (strategy === 'Fixed') {
    ctx.pushStatement(`body.push(...(${formatBodyArray(node.config.fixedBody)} as BodyPartConstant[]));`);
  } else if (strategy === 'Weighted') {
    ctx.pushStatement(`const weightConfig = ${toObjectLiteral(weights)} as Record<string, number>;`);
    ctx.pushStatement('const costs: Record<string, number> = {');
    ctx.enterScope();
    ctx.pushStatement('MOVE: 50, WORK: 100, CARRY: 50, ATTACK: 80, RANGED_ATTACK: 150, HEAL: 250, TOUGH: 10, CLAIM: 600');
    ctx.exitScope();
    ctx.pushStatement('};');
    const energyExpr = energyCap > 0 ? String(energyCap) : 'spawn.room.energyCapacityAvailable';
    ctx.pushStatement(`let energy = ${energyExpr};`);
    ctx.pushStatement('const entries = Object.entries(weightConfig).filter(([, value]) => typeof value === "number" && value > 0);');
    ...
    ctx.pushStatement('for (let i = 0; i < weight && energy >= cost; i += 1) {');
    ctx.enterScope();
    ...
    ctx.exitScope();
    ctx.pushStatement('}');
    ctx.pushStatement(`if (body.length === 0) { body.push(...(${formatBodyArray(node.config.fixedBody)} as BodyPartConstant[])); }`);
  } else {
    ...
    ctx.pushStatement('4: [WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE],');
    ctx.pushStatement('5: [WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE],');
    ctx.pushStatement('6: [WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE],');
    ctx.pushStatement('7: [WORK, WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE],');
    ctx.pushStatement('8: [WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE];');
    ctx.exitScope();
    ctx.pushStatement('};');
  ...
  ctx.pushStatement(`const memory = ${toObjectLiteral(memoryTemplate)} as CreepMemory;`);
  ctx.pushStatement('if (memory.role == null && memory.roleName) { memory.role = memory.roleName as string; }');
...
export const emitTowerLogic = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  ...
  ctx.enterScope();
  ctx.pushStatement('const hostile = tower.pos.findClosestByRange(FIND_HOSTILE_CREEPS);');
...
export const emitLinkSend = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  const from = ctx.resolvePort(node.id, node.inputs.from, 'undefined');
  const to = ctx.resolvePort(node.id, node.inputs.to, 'undefined');
  const amountExpr = node.config.amount !== null && node.config.amount !== undefined ? `${Number(node.config.amount)}` : 'undefined';
  ctx.pushStatement(`if (${from} && ${to}) {`);
  ctx.enterScope();
  ctx.pushStatement(`const result = (${from} as StructureLink).transferEnergy(${to} as StructureLink, ${amountExpr});`);
  ctx.pushStatement('void result;');
  ctx.exitScope();
  ctx.pushStatement('}');
  node.outputs.forEach((targetId) => emitNext(targetId));
...
export const emitTerminalMarket = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  const terminalExpr = ctx.resolvePort(node.id, node.inputs.terminal, 'creep.room.terminal');
  const resource = typeof node.config.resource === 'string' ? node.config.resource : 'RESOURCE_ENERGY';
  ...
  ctx.pushStatement(`const terminal = ${terminalExpr} as StructureTerminal | undefined;`);
  ...
  if (mode === 'buy') {
    ...
    ctx.pushStatement('const orderId = (terminal as any).memory?.orderId as Id<Order> | undefined;');
    ctx.pushStatement(
