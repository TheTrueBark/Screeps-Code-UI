...
const buildFilterPredicate = (filters: unknown): string => {
  ...
  const conditions = filters
    .map((filter) => {
      ...
      const { field, op, value } = filter as { field?: string; op?: string; value?: unknown };
      const left = accessor('candidate', field ?? '');
      const operator = typeof op === 'string' && op.trim().length > 0 ? op : '===';
      const right = typeof value === 'string' && !value.trim().startsWith('Game') && !value.trim().startsWith('creep')
...
export const emitFind = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  let roomExpr = 'creep.room';
  if (node.config.roomScope === 'name') {
    const configured = typeof node.config.roomName === 'string' && node.config.roomName.trim().length > 0
      ? JSON.stringify(node.config.roomName.trim())
      : 'creep.room.name';
    roomExpr = `Game.rooms[${configured}] ?? creep.room`;
  }

  const findConstant = typeof node.config.findConstant === 'string' ? node.config.findConstant : 'FIND_SOURCES';
  const sourceVar = ctx.newTemp('found');
  ctx.pushStatement(`const ${sourceVar} = (${roomExpr})?.find(${findConstant}) ?? [];`);

  const filters = buildFilterPredicate(node.config.filters);
  const filteredVar = ctx.newTemp('filtered');
  ctx.pushStatement(`const ${filteredVar} = ${sourceVar}.filter((candidate) => ${filters});`);

  const limit = Number(node.config.limit ?? 0);
  const limitedVar = ctx.newTemp('limited');
  if (limit > 0) {
    ctx.pushStatement(`const ${limitedVar} = ${filteredVar}.slice(0, ${limit});`);
  } else {
...
export const emitResolveById = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  const idExpr = ctx.resolvePort(node.id, node.inputs.id, node.config.id ?? 'undefined');
  const varName = ctx.newTemp('resolved');
  ctx.pushStatement(`const ${varName} = Game.getObjectById(${idExpr} as Id<any>);`);
  ctx.setValue(node.id, varName);
  node.outputs.forEach((target) => emitNext(target));
};

export const emitNeighborhood = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  const radius = Number(node.config.radius ?? 0) || 0;
  const lookType = typeof node.config.type === 'string' ? node.config.type : 'LOOK_TERRAIN';
  if (node.config.mode === 'lookForAtArea') {
    ...
    ctx.pushStatement(`const ${filteredVar} = ${rawVar}.filter((entry) => entry.type === ${lookType});`);
    ctx.setValue(node.id, filteredVar);
  }
  node.outputs.forEach((target) => emitNext(target));
};

export const emitSortBest = (node: NodeIR, ctx: EmitContext, emitNext: (nextId: string) => void) => {
  ...
  const strategy = typeof node.config.strategy === 'string' ? node.config.strategy : 'closestByPath';
  const resultVar = ctx.newTemp('best');

  if (strategy === 'closestByRange') {
    ctx.pushStatement(`const ${resultVar} = creep.pos.findClosestByRange(${listVar} as any);`);
  } else if (strategy === 'closestByPath') {
    ctx.pushStatement(`const ${resultVar} = creep.pos.findClosestByPath(${listVar} as any);`);
  } else {
    ...
    ctx.pushStatement(`const ${resultVar} = ${listVar}.reduce((best, item) => {`);
    ctx.enterScope();
    ...
    ctx.pushStatement(`if (${comparison === '<' ? 'current < bestValue' : 'current > bestValue'}) {`);
    ctx.enterScope();
    ctx.pushStatement('return item;');
    ctx.exitScope();
    ctx.pushStatement('}');
