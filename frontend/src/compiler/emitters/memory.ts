...
const scopeAccess = (scope: unknown): { base: string; init?: string } => {
  switch (scope) {
    case "room":
      return {
        base: "Memory.rooms[creep.room.name]",
        init: "Memory.rooms[creep.room.name] = Memory.rooms[creep.room.name] ?? {} as any;",
      };
    case "creep":
      return {
        base: "Memory.creeps[creep.name]",
        init: "Memory.creeps[creep.name] = Memory.creeps[creep.name] ?? {} as any;",
      };
    default:
      return { base: "Memory" };
  }
...
const ensureIntermediateObjects = (
  ctx: EmitContext,
  base: string,
  path: string[],
) => {
...
export const emitMemoryRead = (
  ...
  const fallback =
    typeof node.config.defaultValue === "string"
...
export const emitMemoryWrite = (
  ...
  if (node.config.merge) {
    ctx.pushStatement(
      `${targetAccess} = { ...(${targetAccess} ?? {}), ...(${valueExpr}) };`,
    );
  } else {
