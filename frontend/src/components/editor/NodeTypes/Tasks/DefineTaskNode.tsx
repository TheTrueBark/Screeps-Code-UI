...
const baseDefinition: Omit<NodeDefinition, 'Component'> = {
  ...
  title: 'Define Task',
  subtitle: 'Tasks',
  description: 'Create a named callable task with its own entry branch and parameter schema.',
  family: 'task',
  category: 'Tasks',
  defaultConfig: {
    taskName: 'refill',
    params: [
      { key: 'min', type: 'number', default: 50 }
    ]
  },
  configFields: [
    { type: 'text', name: 'taskName', label: 'Task name', placeholder: 'refill' },
    {
      ...
      name: 'params',
      label: 'Parameters',
      helper: 'Array of { key, type, default } objects. Allowed types: ' + TASK_PARAM_TYPES.map((option) => option.value).join(', ')
    }
  ],
