...
const resourceOptions = RESOURCES.map((resource) => ({ label: resource.replace('RESOURCE_', ''), value: resource }));
...
const baseDefinition: Omit<NodeDefinition, 'Component'> = {
  ...
  configFields: [
    ...
    { type: 'select', name: 'resource', label: 'Resource', options: resourceOptions },
    { type: 'number', name: 'price', label: 'Price', min: 0, step: 0.01 },
    { type: 'number', name: 'amount', label: 'Amount', min: 0, step: 100 }
  ],
  dataInputs: [
    { name: 'terminal', label: 'Terminal', handleId: 'input:terminal', optional: true }
  ]
};
