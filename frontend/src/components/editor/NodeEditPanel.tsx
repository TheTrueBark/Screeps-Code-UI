...
import { FILTER_OPERATORS, normalizeFilters, safeJsonValue } from "./NodeTypes/BaseNode";
...
export const NodeEditPanel = ({
    ...
    (updater: (prev: Record<string, unknown>) => Record<string, unknown>) => {
      ...
      setNodes((current) =>
        current.map((entry) => {
          ...
          const currentData = (entry.data as ScreepsNodeData) ?? data ?? FALLBACK_DATA;
  ...
  const handleFilterChange = useCallback(
    (id: string, key: "field" | "op" | "value", value: string) => {
      setConfig((prev) => {
        const current = normalizeFilters((prev as Record<string, unknown>).filters);
        return {
          ...prev,
  ...
  const handleAddFilter = useCallback(() => {
    setConfig((prev) => {
      ...
      return {
        ...prev,
        filters: [...current, { id: nanoid(6), field: "", op: "===", value: "" }],
      };
    });
  }, [setConfig]);
  ...
  const headerIcon = useMemo(() => {
    ...
    if (meta?.acronym) {
      return <span className="node-edit-panel-icon-fallback">{meta.acronym}</span>;
    }

    if (definition?.title) {
      return <span className="node-edit-panel-icon-fallback">{definition.title.slice(0, 2).toUpperCase()}</span>;
    }
      ...
      <div className="node-edit-panel-header">
        <div className="node-edit-panel-header-main">
          ...
          <div className="node-edit-panel-titles">
            <div className="node-edit-panel-title">{meta?.title ?? data?.label ?? "Node"}</div>
            <div className="node-edit-panel-subtitle">{definition?.description ?? ""}</div>
          </div>
        </div>
        <button type="button" className="node-edit-panel-close" onClick={onClose} aria-label="Close editor">
          ✕
        </button>
      </div>
      <div className="node-edit-panel-content">
        {!hasDefinition ? (
          <div className="node-edit-panel-empty">Select a node and choose Edit to configure it.</div>
        ) : configFields.length === 0 ? (
          <div className="node-edit-panel-empty">This node has no editable settings.</div>
        ) : (
          configFields.map((field) => {
            ...
            if (field.type === "json" && field.name === "filters") {
              return (
                <div key={`config:${field.name}`} className="node-edit-panel-section">
                  <div className="node-edit-panel-label">{field.label}</div>
                  {field.helper ? <div className="node-edit-panel-helper">{field.helper}</div> : null}
                  <div className="node-edit-panel-filters">
                    {filters.length === 0 ? (
                      <div className="node-edit-panel-empty muted">No filters defined.</div>
                    ) : (
                      filters.map((filter, index) => (
                        <div key={filter.id} className="node-edit-panel-filter">
                          <div className="node-edit-panel-filter-heading">{buildFilterSummary(filter, index)}</div>
              ...
              return (
                <div key={`config:${field.name}`} className="node-edit-panel-section">
                  ...
                  <input
                    ...
                    step={field.step}
                    onChange={(event) =>
                      handleFieldChange(field.name, event.target.value === "" ? null : Number(event.target.value))
                    }
                  />
                </div>
              );
